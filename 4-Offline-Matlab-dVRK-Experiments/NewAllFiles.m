...
% Flag_DanMaster !!! % Process_Dan_Master p value change
...
if Flag_DanMaster == 0
    ...
    dVRK_Master_Right = "Master_Right.txt";
    dVRK_Master_Left = "Master_Left.txt";

    dVRK_Clutching = "Clutching.txt";
end


if Flag_DanMaster == 0
    Master_Right = 'dvrk_Master_Right.txt';
    Master_Left = 'dvrk_Master_Left.txt';

    Clutching = 'dvrk_Master_Clutching.txt';

else
    Master_Right = 'Master_Right.txt';
    Master_Left = 'Master_Left.txt';

    Clutching = 'Clutching.txt';
end
    ...
    if img_num > 0
        for j = 1:img_num
            ...
            if image_name == dVRK_Master_Right 
                phns_Master_Right=fullfile(file_path,Master_Right);
                Read_dVRK_Master_Right = 1;
            end
            
            if image_name == dVRK_Master_Left
                phns_Master_Left=fullfile(file_path,Master_Left);
                Read_dVRK_Master_Left = 1;
            end
         
            if image_name == dVRK_Clutching
                Read_Clutch = 1;
                f=fullfile(file_path,Clutching);
        ...
        if Read_Right == 1 && Read_Left == 1 && Read_dVRK_Master_Right == 1 && Read_dVRK_Master_Left == 1 && Read_Clutch == 1
            if Flag_DanMaster == 0
            [Total_Slave_Right_Distance,Total_Slave_Left_Distance,Total_Master_Right_Distance,Total_Master_Left_Distance]=Process_Overall(phns_Slave_Right,phns_Slave_Left,phns_Master_Right,phns_Master_Left,Out_Put,i,Flag_DanMaster);                  
            else
            [Total_Slave_Right_Distance,Total_Slave_Left_Distance,Total_Master_Right_Distance,Total_Master_Left_Distance] = Process_Dan_Master(phns_Slave_Right,phns_Slave_Left,phns_Master_Right,phns_Master_Left,Out_Put,i,Flag_DanMaster);
            end
