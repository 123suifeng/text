    ...
    if img_num > 0
        for j = 1:img_num
            ...
            if image_name == dVRK_Master_Right 
                phns_Master_Right=fullfile(file_path,'Master_Right.txt');
                Read_dVRK_Master_Right = 1;
            end
            
            if image_name == dVRK_Master_Left
                phns_Master_Left=fullfile(file_path,'Master_Left.txt');
                Read_dVRK_Master_Left = 1;
            end
         
            if image_name == dVRK_Clutching
                Read_Clutch = 1;
                f=fullfile(file_path,'Clutching.txt');
        ...
        if Read_Right == 1 && Read_Left == 1 && Read_dVRK_Master_Right == 1 && Read_dVRK_Master_Left == 1 && Read_Clutch == 1
            [Total_Slave_Right_Distance,Total_Slave_Left_Distance,Total_Master_Right_Distance,Total_Master_Left_Distance]=Process_Dan_Master(phns_Slave_Right,phns_Slave_Left,phns_Master_Right,phns_Master_Left,Out_Put,i,1);                  
